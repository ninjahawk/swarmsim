...
90 findings documented — full evidence and figures: [`findings.md`](findings.md)
...
| 90 | **Two-sided arms race: capture ≠ disruption, and the race is *asymmetric*** *(closes the co-adaptation thread)*. Now the predator co-evolves a heritable predictive **lead time**, selected on capture success (replace-worst-with-mutated-best). (i) Against *frozen* no-escape prey the lead converges tightly to **~3.0**, **not** the F66 hand-found most-*disruptive* ~2 — selection optimises **captures, not coherence-disruption** (fragmenting *scatters* prey; a longer lead intercepts the flock's path for more kills): distinct objectives, distinct optima. (ii) The race is asymmetric — the predator climbs to its optimum from any start, but the prey counter is **origination-limited** (F87/F88), so de-novo co-evolution **favours the predator** (prey stall at w~0.7); from *seeded* escape, escape **fixes** (w~2.0, frac 0.99) and the predator's lead **drifts** (no captures → no selection signal). Reconciles F89: the ~60% free-rider equilibrium needs a *persistently effective* predator; once the predator decays, escape fixes. Collective escape is a **powerful but evolutionarily fragile** defence — hard to originate, easy to lose to drift, decisive once present. (2–3-seed noise; signs robust) |
...
| `evolution/` | co-adaptation — heritable per-agent escape weight under capture/removal selection vs the predictive predator; the F70 valley as an evolutionary brake (F87) that blocks origination but not invasion (F88); robust free-rider equilibrium (F89); two-sided arms race with a co-evolving predator (F90). Builds on the validated `vectorized_predator.py` + `vectorized_predator_prey.py` harness. | `escape_evolution.py`, `escape_invasion.py`, `escape_freerider.py`, `escape_coevolution.py` |
...
| `findings.md` | Running notes on all 90 findings with figures |
