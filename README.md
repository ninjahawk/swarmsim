...
89 findings documented — full evidence and figures: [`findings.md`](findings.md)
...
| `evolution/` | co-adaptation — heritable per-agent escape weight under capture/removal selection vs the predictive predator; the F70 valley as an evolutionary brake (F87) that blocks origination but not invasion (F88). Builds on the validated `vectorized_predator.py` + `vectorized_predator_prey.py` harness. | `escape_evolution.py`, `escape_invasion.py`, `escape_freerider.py` |
...
| `findings.md` | Running notes on all 89 findings with figures |
