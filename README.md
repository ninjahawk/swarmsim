...
| `evolution/` | co-adaptation — heritable per-agent escape weight under capture/removal selection vs the predictive predator; the F70 valley as an evolutionary brake (F87) that blocks origination but not invasion (F88); robust free-rider equilibrium (F89); two-sided arms race with a co-evolving predator (F90); direct capture-vs-lead diagnostic (F91 self-test, correcting F90); energy-budget robustness check (F92); and a second heritable trait — alignment strength — that is bistable too (F93) but whose high-alignment minority does *not* invade, because alignment is a mutual coupling rather than a free-rideable signal (F94). Builds on the validated `vectorized_predator.py` + `vectorized_predator_prey.py` harness. | `escape_evolution.py`, `escape_invasion.py`, `escape_freerider.py`, `escape_coevolution.py`, `escape_capture_curve.py`, `escape_energy.py`, `evolve_alignment.py`, `alignment_invasion.py` |
